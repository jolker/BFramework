package com.bliss.framework.redis;
...
public class RedisFactory {
    ...
    private void initPool(String poolName) {
        try {
            ...
            String host = ConvertUtils.toString(Config.getParam(poolName, "host"), "127.0.0.1");
            String password = ConvertUtils.toString(Config.getParam(poolName, "password"), null);

            int port = ConvertUtils.toInt(Config.getParam(poolName, "port"), 6379);
            int timeout = ConvertUtils.toInt(Config.getParam(poolName, "timeout"), Protocol.DEFAULT_TIMEOUT);
            int maxActive = ConvertUtils.toInt(Config.getParam(poolName, "max_active"), 8);
            int maxIdle = ConvertUtils.toInt(Config.getParam(poolName, "max_idle"), 8);
            int minIdle = ConvertUtils.toInt(Config.getParam(poolName, "min_idle"), 8);
