...
import com.bliss.framework.ipchecking.IPRange;
...
public class NetworkUtils {
    ...
    public static IPRange ipRangeToRangeNumber(String ipRange) {
        try {
            String[] arr = ipRange.split("/");
            if (arr.length == 1) {
                long value = ipToLong(arr[0]);
                return new IPRange(value, value);
            } else if (arr.length == 2) {
                int len = ConvertUtils.toInt(arr[1]);
