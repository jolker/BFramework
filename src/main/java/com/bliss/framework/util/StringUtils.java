...
//                    String.format("Type %s is not supported (yet)", clazz.getName());
     ...
     *
     * @param valid a-z,A-Z, 0-9 ,_
     * @return true/false
     */
    public static boolean checkSpecialCharacter(String text) {
        Pattern p = Pattern.compile("^[a-zA-Z0-9]{4,16}+([a-zA-Z0-9]+)*$", Pattern.CASE_INSENSITIVE);
     ...
     * @return 
     */
    public static String normalize(String input, int minLenght, int maxLength){
        String newString = new String(input);
        newString = normalize(newString);
        if(newString.length() < minLenght){
            newString = newString + getRandomString((minLenght - newString.length()) + 1);
        }
        if(newString.length() > maxLength){
            newString = org.apache.commons.lang3.StringUtils.substring(newString, 0, maxLength);
        }
        return newString;
    }
}
